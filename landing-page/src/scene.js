...
function scene() {
  // Controls the opacity of the helper plane and sphere
  ...
  const pGeometry = new THREE.PlaneGeometry(
    ...
    1,
    1
  ).rotateX(Math.PI / 2).translate( 0, -4, 0 );
  ...
  function modifyPointHeight(distance) {
    ...
    const amplitude = 6.0;
    return (
      amplitude * Math.exp(-Math.pow(distance, 2) / (2 * Math.pow(sigma, 2)))
    );
  }
  ...
  let intersectionPoint = null;  // Declare it globally
  ...
  //       intersectionPoint = intersects[0].point;  // Update the global intersectionPoint
  ...
  //     }
  //   }, 100)  // Increased throttle to 100ms
  // );

  // Unthrottled mousemove event listener
